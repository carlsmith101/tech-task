...
def test_PUT_article_status_code_is_400():
    response = requests.put(url=BASE_URL)
    assert response.status_code == 400 and response.text == '"Invalid request"'
...
def test_DELETE_article_status_code_is_400():
...
def test_POST_article_status_code_is_400():
    response = requests.post(url=BASE_URL + "1", data='')
    assert response.status_code == 400 and response.text == '"Invalid request"'
